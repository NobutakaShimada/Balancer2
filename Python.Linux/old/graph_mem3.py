...
class MemoryViewerApp:
    def __init__(self, root):
        ...
        self.readonly_variables = {"GAIN_OPTION6"}
        ...
        self.editing_start_time = 0  # 編集開始時刻を記録
        ...
        # 外部クリックのイベントバインドを削除（問題の原因）
        # self.root.bind('<Button-1>', self._handle_tree_click_outside_edit, True)
    ...
    def _on_value_click(self, item):
        ...
        if item in self.readonly_variables:
            print(f"{item} は読み取り専用です")
            messagebox.showinfo("情報", f"{item} は読み取り専用です")
            return
    ...
    def _start_edit(self, item):
        ...
        self.editing_start_time = time.time()  # 編集開始時刻を記録
        ...
        try:
            self.edit_entry = tk.Entry(self.tree, relief='solid', borderwidth=2, 
                                     bg='lightyellow' if hasattr(tk.Entry, 'bg') else None)
    ...
    def _handle_tree_click_outside_edit(self, event):
        """編集領域外クリック時の処理"""
        # 編集中のEntryウィジェット上のクリックは無視
        if self.editing_item and self.edit_entry:
            # クリック位置がEntryウィジェット内かチェック
            try:
                entry_x = self.edit_entry.winfo_x()
                entry_y = self.edit_entry.winfo_y()
                entry_width = self.edit_entry.winfo_width()
                entry_height = self.edit_entry.winfo_height()
                
                # ウィンドウ内での絶対座標に変換
                abs_x = event.x_root - self.root.winfo_rootx()
                abs_y = event.y_root - self.root.winfo_rooty()
                
                # Entryウィジェット領域内のクリックは無視
                if (entry_x <= abs_x <= entry_x + entry_width and 
                    entry_y <= abs_y <= entry_y + entry_height):
                    print("Entry内クリック - 編集継続")
                    return
                    
            except Exception as e:
                print(f"座標計算エラー: {e}")
            
            print("Entry外クリック - 編集終了")
            self._end_edit(commit=False)
