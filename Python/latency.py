...
# decorator for latency measurement
#import latency_decorator as ld
import LatencyMeasurer as ld
...
if __name__ == '__main__':
    with open('/dev/BeuatoCtrl0', mode='r+b', buffering=0) as dev:
        ...
        try:
            iterations = 100
            print(f"レイテンシの計測を始めます({iterations}回)...")
            ...
            for i in range(iterations):
                ...
                if (i+1)%10 == 0:
                    ...
                    print(f"  最新レイテンシ - BODY_ANGULAR_SPD: {timing2['latency_ms']:.3f}ms")
                    print(f"  最新レイテンシ - WHEEL_ANGLE_L: {timing3['latency_ms']:.3f}ms")
                    print(f"  最新レイテンシ - GYRO_DATA: {timing4['latency_ms']:.3f}ms")
                #print(struct.unpack_from("<H", payload,24)[0])

            stats = ld.LatencyMeasurer.print_stats()
        except KeyboardInterrupt:
            print("\n計測を中断しました")
            ld.LatencyMeasurer.print_stats()
